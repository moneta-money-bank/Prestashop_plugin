...
include_once _PS_MODULE_DIR_ . '/moneta/translations/statuses.php';

class Moneta extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Moneta');
        $this->description = $this->l('Accepts payments by Moneta');
    ...
    public function displayForm()
    {
        $form['moneta'] = [
            ...
            $form['merchant'] = [
                'form' => [
                    'legend' => [
                        'title' => $this->l('Moneta Connection settings'),
                        'icon' => 'icon-cog'
                    ],
                    'input' => [
                        [
        ...
        $this->addIntegrationMode($form['moneta']['form']['input']);
    ...
    public function buildTemplatePath($name)
    {
        ...
        return 'module:moneta/views/templates/front/' . $name . '17.tpl';
    }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        $paymentOption->setCallToActionText($this->l('Pay with Moneta'))
...
//         $langCode = (new Language($order->id_lang)) -> iso_code;
        ...
        if (Configuration::get('EVO_REFUND_POSSIBILITY') && $order->module == 'moneta') {
            $lastStatus = 0;
            ...
            if ($orders) {
                foreach ($orders as &$orderr) {
                    ...
                    if($rawInfo !== ""){
                        ...
                        $orderr['info'] = $this->l('Refunded: ', 'moneta').$rawInfo;
                    }
    ...
    private function refundProcess($order, $tokenEvo)
    {
        ...
        if($possibleToRefund < 0 || round($refundAmount,2) > round($possibleToRefund,2)){
            return ['state' => 'danger', 'text' => $this->l('You can not make a refund on this amount for this order.', 'moneta')];
        }
        ...
        if(!$refund->errors){
            ...
            //$order->addOrderPayment('-'.$refundAmount, 'Moneta', $id_evo_payment, $currency, date('Y-m-d H:i:s'));
            ...
            return ['state' => 'danger', 'text' => $this->l('No connect with payment gateway.', 'moneta')];
        }
        return false;
    }
    ...
    private function getSumCurrentOrderPayments($order)
    {
        ...
        foreach ($payments as $payment) {
            if($payment->payment_method=='Moneta'){
    ...
    {
        $link = $this->context->link->getModuleLink('moneta', 'payment');
    ...
    {
        $paymentOptions = [
            'cta_text' => $this->l('Pay with Moneta'),
            'action' => $this->context->link->getModuleLink('moneta', 'payment'),
    ...
    public function getPaymentToken($merchantTxId, $retry='', $mode, $amount='')
    {
        ...
        try {
           ...
           $notificationUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=moneta&controller=response&ipg=0&retry=' . $retry;
//             debug locally
//              $notificationUrl = 'http://externalip:8866/presta175/index.php?fc=module&module=moneta&controller=response&ipg=1&retry=' . $retry;
            $landingUrl = Tools::getHttpHost(true).__PS_BASE_URI__ . 'index.php?fc=module&module=moneta&controller=response&ipg=0&retry=' . $retry;
    ...
    public function addNewOrderState($state, $names)
    {
        if (!(Validate::isInt(Configuration::get($state)) && Validate::isLoadedObject($order_state = new OrderState(Configuration::get($state))))) {
            ...
            $order_state->module_name = 'moneta';
    ...
    /**
     * get payment status description by Moneta status
     */
